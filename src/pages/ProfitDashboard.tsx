...
const ProfitDashboard: React.FC = () => {
  ...
  try {
    const existing = await fetch("http://localhost:5000/api/profit-ledger");
    ...
    await fetch("http://localhost:5000/api/profit-ledger/bulk-insert", {
      method: "POST",
  ...
  const exportExpenseXLSX = () => {
    const data = filteredExpense.map((row) => ({
      Date: row.created_at,
